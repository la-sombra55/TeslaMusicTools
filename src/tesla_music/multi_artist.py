...
def split_multi_artist(artist):
    ...
    a multi-artist credit joined by '&', ',', 'and', '/', or 'vs'.
    Returns None when fewer than two names are found.
    """
    normalized = re.sub(r"\s*&\s*", ", ", artist)
    normalized = re.sub(r"\s*,?\s*\band\b\s+", ", ", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"\s*/\s*", ", ", normalized)
    normalized = re.sub(r"\s+vs\.?\s+", ", ", normalized, flags=re.IGNORECASE)
